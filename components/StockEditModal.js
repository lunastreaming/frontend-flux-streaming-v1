...
export default function StockEditModal({
  ...
  onClose,
  onSuccess,
  initialData = null, // stock activo (requiere al menos: id)
  ...
  const [form, setForm] = useState({
    ...
    numeroPerfil: '',
    pin: '',
    note: '' // nota opcional del proveedor
  })
  ...
  useEffect(() => {
    ...
    if (initialData) {
      setForm({
        ...
        numeroPerfil: (initialData.numeroPerfil ?? initialData.numberProfile ?? '') === '' ? '' : Number(initialData.numeroPerfil ?? initialData.numberProfile),
        pin: initialData.pin ?? '',
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!stockId) {
        alert('No se encontró el id del stock. No se puede editar.')
        setSubmitting(false)
        return
      }
      ...
      const res = await fetch(`${BASE_URL}/api/stocks/${encodeURIComponent(stockId)}`, {
        method: 'PUT',
      ...
      console.error('Error editando stock:', err)
      alert('No se pudo editar el stock: ' + (err.message || err))
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return ReactDOM.createPortal(
    <div
      ...
      data-testid="stockeditmodal-backdrop"
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Editar stock activo"
        style={styles.modal}
        onMouseDown={(e) => e.stopPropagation()}
        data-testid="stockeditmodal-dialog"
      >
        <header style={styles.header}>
          <h3 style={styles.title}>✏️ Editar stock activo</h3>
          <button type="button" onClick={handleClose} aria-label="Cerrar" style={styles.closeBtn} data-testid="stockeditmodal-close"><FaTimes /></button>
        </header>

        <div ref={contentRef} style={styles.contentWrap} className="stockedit-modal-scroll">
          <form onSubmit={(e) => { e.preventDefault(); handleSubmit() }} style={styles.formGrid}>

            {/* Producto solo lectura */}
            <div style={{ gridColumn: '1 / -1' }}>
              <label style={styles.label}>Producto</label>
              <div style={styles.readOnlyProduct} data-testid="stockedit-product">
            ...
            <div style={{ gridColumn: '1 / -1' }}>
              ...
              <input
                ...
                placeholder="Usuario"
                style={styles.input}
                data-testid="stockedit-username"
              />
            </div>
            ...
            <div style={{ gridColumn: '1 / -1' }}>
              ...
              <input
                ...
                data-testid="stockedit-password"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                placeholder="1-7"
                style={styles.input}
                data-testid="stockedit-numperfil"
              />
            </div>
        ...
        <footer style={styles.footer}>
          <button type="button" onClick={handleClose} style={styles.cancelBtn} disabled={submitting} data-testid="stockedit-cancel">Cancelar</button>
          <button type="button" onClick={handleSubmit} style={styles.submitBtn(submitting)} disabled={submitting} data-testid="stockedit-approve">
            {submitting ? 'Procesando...' : 'Guardar cambios'}
          </button>
        </footer>

        <style>{`
          .stockedit-modal-scroll { scrollbar-width: thin; scrollbar-color: rgba(155,178,200,0.6) transparent; overflow-x: hidden; }
          .stockedit-modal-scroll::-webkit-scrollbar { height: 10px; width: 10px; }
          .stockedit-modal-scroll::-webkit-scrollbar-track { background: transparent; }
          .stockedit-modal-scroll::-webkit-scrollbar-thumb { background: linear-gradient(180deg, rgba(155,178,200,0.18), rgba(155,178,200,0.28)); border-radius: 999px; border: 2px solid rgba(2,6,23,0.0); }
...
/* ===== estilos (alineados con RequestToSoldModal) ===== */
const styles = {
