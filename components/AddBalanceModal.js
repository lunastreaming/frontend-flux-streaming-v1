...
export default function AddBalanceModal({ open, onClose, onAdd }) {
  ...
  useEffect(() => {
    if (!open) {
      setAmount(''); setCurrency('PEN'); setError(null); setSubmitting(false)
      setRate(null); setRateError(null); setLoadingRate(false)
      setSelectedKey(null); setCopySuccess('')
    } else if (open && currency === 'USD') {
      fetchRate().catch(() => {})
    }
  }, [open])

  useEffect(() => {
    if (open && currency === 'USD' && rate == null && !loadingRate) fetchRate().catch(() => {})
  }, [currency, open])

  async function fetchRate() {
    setRateError(null); setLoadingRate(true)
    try {
      ...
      setRate(num); setLoadingRate(false); return num
    } catch (err) {
      setRate(null); setRateError(err?.message || String(err)); setLoadingRate(false); throw err
    }
  }
  ...
  const submit = async () => {
    ...
    if (currency === 'PEN') {
      ...
      if (minUsd == null) { setError('Tipo de cambio inválido.'); return }
      if (Number(parsed.toFixed(2)) < minUsd) { setError(`El monto mínimo es ${minUsd.toFixed(2)} USD`); return }
    }
  ...
  const minDisplay = (() => {
    ...
    if (currency === 'USD') {
      ...
      return `${getMinForCurrencyNumber().toFixed(2)} USD`
    }
    return ''
  })()

  // Asegúrate de tener los archivos en /public/images/
  ...
  return (
    <Modal open={open} onClose={() => { if (!submitting) onClose() }} ariaLabel="Agregar saldo">
      ...
      <style jsx>{`
        ...
        /* Grilla responsive y alineada de métodos */
        .method-list {
          display: grid;
        ...
        /* Logo de PayPal más grande solo en el detalle */
        .paypal-logo-large {
